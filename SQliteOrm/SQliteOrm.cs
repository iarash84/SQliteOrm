    ...
    public class SqLiteOrm
    {
        ...
        public void CreateTable<T>() where T : new()
        {
            ...
            foreach (var property in properties)
            {
                ...
                if (isPrimaryKey && !property.IsAutoGenerated)
                    throw new InvalidOperationException($"Key property '{property.PropertyName}' must be an int or long to use SQLite AUTOINCREMENT.");
        ...
        public int Insert<T>(T obj)
        {
            ...
            var properties = map.Properties.Where(p => !p.IsAutoGenerated)
                .ToArray();

            if (properties.Length == 0)
                throw new InvalidOperationException("No insertable properties were found.");

            var columns = string.Join(", ", properties.Select(p => QuoteIdentifier(p.ColumnName)));
        ...
        public void Insert<T>(List<T> objectList)
        {
            ...
            var map = EntityMapCache.Get<T>();
            var tableName = QuoteIdentifier(map.TableName);
            var properties = map.Properties.Where(p => !p.IsAutoGenerated).ToArray();
            if (properties.Length == 0)
                throw new InvalidOperationException("No insertable properties were found.");

            var query = $"INSERT INTO {tableName} ({string.Join(", ", properties.Select(p => QuoteIdentifier(p.ColumnName)))}) " +
            ...
            lock (_writeLock)
            {
                ...
                try
                {
                    foreach (var obj in objectList)
                    {
                        ...
                        foreach (var property in properties)
                            command.Parameters[$"@{property.PropertyName}"].Value = property.GetValue(obj!) ?? DBNull.Value;
            ...
            LogicalOperator conditionType = LogicalOperator.And) where T : new()
        {
            ...
            var query = $@"SELECT t.*, r.{relatedFieldName} AS {aliasName}
                    ...
                    ON t.{relationFieldName} = r.{QuoteIdentifier(relatedMap.Key?.ColumnName ?? "Id")}";
